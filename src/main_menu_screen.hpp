...
{
public:
    main_menu_screen(sf::RenderWindow *win, screen_manager *sm, const resource_manager *rm) :
        win_(win), screen_manager_(sm), resource_manager_(rm)
    {
        ...
        const sf::Font &font = resource_manager_->acquire<sf::Font>("Pokemon GB");
...
private:
    ...
    {
        screen_manager_->push_screen(std::make_shared<game_screen>(win_, screen_manager_, resource_manager_));
    }
